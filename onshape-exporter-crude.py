...
def get_part_id(api_access, api_secret, DID, WID, EID, partToExport):
    response = requests.get(
...
def main():
    ...
    PID = get_part_id(API_ACCESS, API_SECRET, DID, WID, EID, partName)

    for export in configurationsToExport:
        resolved_params = resolve_configuration_parameters(export["config"], config_schema)
        encodedId = encode_configuration(API_ACCESS, API_SECRET, DID, EID, resolved_params)
        print(encodedId)
